      program rayleigh
c
c   Ying; replacing : 1.d-6 with eps      

c     this is an adaptation of Guy Master's program sph1.f
...
c compile:  f77 rayleigh.f ltable.r
c source text of several subroutines is ratfor, in ltable.r
...
c      stop   
      ...
      close(8)
      close(9)
c done with dummyeig (ltable) remove it:  (SL Aug 93)
...
c**** of the secular determinant as det and the count of zero crossings.
      ...
      common r(nlmax),fmu(nlmax),flam(nlmax),qshear(nlmax),qkappa(nlmax),
     + xa2(nlmax),xlam(nlmax),rho(nlmax),qro(3,nlmax),g(nlmax),qg(3,nlmax),
     + fcon(nlmax),fspl(3,nlmax),lcon(nlmax),lspl(3,nlmax),ncon(nlmax),
     + nspl(3,nlmax),ccon(nlmax),cspl(3,nlmax),acon(nlmax),aspl(3,nlmax)
     ...
     +      y1(nlmax),dy1dr(nlmax),y2(nlmax),dy2dr(nlmax),dum(3345)
      ...
      dimension ass(5),vf(nlmax),y(4),dwdp(4)
...
c*** propagate a minor vector in a solid region from level jf to jl ***
     ...
     + nspl(3,nlmax),ccon(nlmax),cspl(3,nlmax),acon(nlmax),aspl(3,nlmax)
      common/bits/pi,rn,vn,wn,w,wsq,wray,qinv,cg,tref,fct,eps,fl,
     +  fl1,fl2,fl3,sfl3,nord,l,kount,knsw,ifanis,iback
      common/eifx/ar(5,nlmax),inorm(nlmax),kdum(4906)
   ...
   35     call rkdot(f,s,h,nvesm,ni)
        if(knsw.eq.1) then
          ...
          if(isplit.eq.1) then
c Ying                  
c            if(dabs(dx).lt.1.d-6) goto 200
             if(dabs(dx).lt.eps) goto 200
            do 100 j=1,nvesm
  100         f(j)=s(j)
            dx=dx*0.5d0
            y=x+dx
...
c*** massages spheroidal mode eigenfunctions before output ***
     ...
     + nspl(3,nlmax),ccon(nlmax),cspl(3,nlmax),acon(nlmax),aspl(3,nlmax)
      common/bits/pi,rn,vn,wn,w,wsq,wray,qinv,cg,tref,fct,eps,fl,
     +  fl1,fl2,fl3,sfl3,nord,l,kount,knsw,ifanis,iback
      common/eifx/a(5,nlmax),inorm(nlmax),kdum(4906)
      common/rindx/nic,noc,nsl,nicp1,nocp1,nslp1,n
      dimension zi(4)
...
c*** do integrals here
      call quod(lsmin,zi)
      cg=zi(2)/(w*zi(1))
      if(zi(4)*zi(1).lt.0.) nerr=1
      wray=dsqrt(abs(2.d0*zi(4)/zi(1)))
      if(nerr.eq.1) wray=-wray
      qinv=2.d0*zi(3)/(wsq*zi(1))
      rnorm=1.d0/(w*dsqrt(abs(zi(1))))        
      rnorm1=rnorm/sfl3
c*********
...
c*** given rho and spline coeffs,computes gravity ***
      implicit real*8(a-h,o-z)
      dimension g(nlmax),rho(nlmax),qro(3,nlmax),r(nlmax)
      g(1)=0.d0
      do 10 i=2,n
        im1=i-1
        del=r(i)-r(im1)
...
c chukren added for test
      write(*,905) (i,r(i),rho(i),i=1,n)
  905 format(3x,i3,f12.4,f12.2)
   ...
   25 ls=jf
      ...
      subroutine modeln(iin,iout,ioeig)
      parameter(nlmax=700)
      implicit real*8(a-h,o-z)
      integer*4 ititle(20)
...
c chukren added for test
c      write(*,905) (i,r(i),rho(i),vpv(i),vph(i),vsv(i),vsh(i),
...
c note Guust: I don't think this is needed for numerical stability
c and it complicates things horribly, but I leave it in so as to avoid
c any errors due to conversion. The 'funny units' are:
c length: Earth radius (6371000 m)
c density: average density of the Earth (5515 kg/m^3)
c acceleration: 7.3651 m/s^2 (see gn)
c velocity: 6850 m/s
c frequency: 0.001/s
      rn=r(n)
      ...
      wn=vn/rn
      do 45 i=1,n
c chukren     if(i < 100)  write(*,*) i,r(i),rn,r(i)/rn
      r(i)=r(i)/rn
c chukren: runtime error when i-1 = 0       
      if(i.gt.1) then 
...
c   for rspln.
c                                                     -rpb
      dimension x(nlmax),y(nlmax),q(3,nlmax),f(3,nlmax),yy(3)
      equivalence (yy(1),y0)
      data yy/3*0.d0/
      j1=i1+1
      y0=0.d0
...
c*** performs dot product with rks coefficients ***
   ...
   30 f(j)=s(j)+b(43)*(h(j,1)+h(j,10)+b(44)*(h(j,4)+h(j,6))+
     +b(45)*h(j,5)+b(46)*(h(j,7)+h(j,9)))
      ...
      implicit real*8(a-h,o-z)
c      common/mtab/we(1200),de(1200),ke(1200),wtry(600),bm(600),um(600)
      common/mtab/we(2400),de(2400),ke(2400),wtry(1200),bm(1200),um(1200)
