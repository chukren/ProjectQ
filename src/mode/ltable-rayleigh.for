...
c
c  open(90,file='layer.rayleigh') for wkern.for, read-engi.for disperion curves ...
c  recorded the layers of engi-functions and the phase velocity...
c  => w  vphase gcom number-layer stratl endl 
c
c to calc low order l<2
c  change 10.0d to 2.0d  or 1.0d  
c
c
...
23027 continue
      ...
      do23028 m=mmin,mmax 
c Ying: for each m, if k1(m)=k2(m)+1 jump out of the loop
c if not, update fl -> x2 k2 and d2      
...
23050 continue
c Ying edit     
      if(abs(wdiff).le.0.001)goto 23052
      write(iout,190) m,ichar(mtype),flroot,fhz,1.0/fhz,vphase,gcom,qmod
...
c      
      number = n-ls00 +1
c Ying edit      
      if(.not.(abs(wdiff).gt.0.001)) write(90,91)w4,vphase,gcom,number,ls00,n
